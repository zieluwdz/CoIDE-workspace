...
{
	char type = 0u;
	uint8_t ret = SD_Init(&type);

	//test 1 block
	char tmp[1024u];
	char *pTmp = &tmp[0u];
	memset(pTmp, 0x0A,1024u);
	memset(pTmp, 0xBC,300u);

	ret = SD_writeMultipleBlocks(pTmp,2u,2u);

	memset(pTmp, 0x00,1024);
	ret = SD_readSingleBlock(pTmp,2u);
	ret = SD_readSingleBlock(pTmp+512u,3u);
	pTmp++;
