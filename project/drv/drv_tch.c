...
void drv_tcs_get_point(char num, uint64_t *pAdcTouchX, uint64_t *pAdcTouchY)
{
  ...
  nm=0;
  while(drv_tcs_get_status_touch() && nm<num)	  											// Measure touch x,y 10 times if the Touch kept pressed,
  {
  ...
  if(nm==num)	                      														// if the touch xy successfuly collected,
  {
	...
	uint64_t x = ((An*dif_adc_X)+(Bn*dif_adc_Y)+Cn)/divider;
	memcpy(pAdcTouchX, &x, sizeof(x));

	uint64_t y = ((Dn*dif_adc_X)+(En*dif_adc_Y)+Fn)/divider;
	memcpy(pAdcTouchY, &y, sizeof(y));
  }
}
...
void drv_tcs_calibrate(void)
{
	  ...
	  drv_lcd_plot_mark_hor(25,215,32,222,BRIGHT_RED);                                  // Plot Signe + for Position Calibrate Touch Point1
	  drv_lcd_printStr_hor("<Touch Screen Calibratetion>",45,164,BRIGHT_BLUE,BLACK);   	// Plot Text	Color Bright blue
	  drv_lcd_printStr_hor("        STEP 1 of 3         ",45,144,BRIGHT_BLUE,BLACK);
	  drv_lcd_printStr_hor(" Click on center of '+' icon",45,104,WHITE,BLACK);        	// Plot Text	Color Black
	  drv_lcd_printStr_hor(" for Touch screen calibrate ",45,84,WHITE,BLACK);
	  while(touch_mark_point != 3)	        											// Number Mark Point Calibrate = 3 Point
	  {
	    if(drv_tcs_get_status_touch())                 													// return (1) if touch screen is detected.
	    {
		  for(n=0;n<50;n++)
		  {
		  ...
		  while(drv_tcs_get_status_touch());        													// Waite until not Touch screen
	      switch(touch_mark_point)																// Select Caribrate Next Step 2 and 3
		  {
		    case 1: drv_lcd_bg_color(BLACK);                                                   	// Clear Screen
		    		drv_lcd_plot_mark_hor(280,120,287,127,BRIGHT_RED);                                  // Plot Signe + for Position Calibrate Touch Point2

	                drv_lcd_printStr_hor("<Touch Screen Calibratetion>",45,164,BRIGHT_BLUE,BLACK);  // Plot Text Color Bright blue
	                drv_lcd_printStr_hor("        STEP 2 of 3         ",45,144,BRIGHT_BLUE,BLACK);
	                drv_lcd_printStr_hor(" Click on center of '+' icon",45,104,WHITE,BLACK);        // Plot Text Color white
	                drv_lcd_printStr_hor(" for Touch screen calibrate ",45,84,WHITE,BLACK);
			        break;

			case 2:	drv_lcd_bg_color(BLACK);                                                         	//Clear Screen
					drv_lcd_plot_mark_hor(153,24,160,31,BRIGHT_RED);                                    //Plot Signe + for Position Calibrate Touch Point3

					drv_lcd_printStr_hor("<Touch Screen Calibratetion>",45,164,BRIGHT_BLUE,BLACK);  //Plot Text	Color Bright blue
					drv_lcd_printStr_hor("        STEP 3 of 3         ",45,144,BRIGHT_BLUE,BLACK);
					drv_lcd_printStr_hor(" Click on center of '+' icon",45,104,WHITE,BLACK);        //Plot Text	Color white
					drv_lcd_printStr_hor(" for Touch screen calibrate ",45,84,WHITE,BLACK);
					break;

			case 3: drv_lcd_bg_color(BLACK);                                                         	//Clear Screen

					drv_lcd_printStr_hor("<Touch Screen Calibratetion>",45,159,BRIGHT_BLUE,BLACK);  //Plot Text	Color Bright blue
					drv_lcd_printStr_hor("          Succeded!         ",45,119,BRIGHT_GREEN,BLACK);
					drv_lcd_printStr_hor("   -Tap Screen for exit-    ",45,79,WHITE,BLACK);

					while(!drv_tcs_get_status_touch());
					delay_ms(100u);
					drv_lcd_bg_color(BLACK);                                                         	// Clear Screen
		            break;
		  }
		}
	    delay_ms(100u);
