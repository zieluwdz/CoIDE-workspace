...
static void 			delay_ms			(unsigned long ms);	// Delay Time
/* Function Control Graphic LCD ET-TFT240320TP-3.2 Rev.B (Driver SPFD5408A)*/
static unsigned char	GLCD_SPI_Read_Write	(unsigned char DataByte);
static void 			GLCD_Write_Command	(unsigned char GLCD_Command);
static void 			GLCD_Write_Data		(unsigned int GLCD_Data);
...
/******************************************/
 ...
 void drv_lcd_printStr_hor(char *str,long cur_x,long cur_y,long fg_color,long bg_color)
 {
   ...
   for (i=0; str[i] != '\0'; i++)
   {
     drv_lcd_text_7x11_hor(str[i],cur_x,cur_y,fg_color,bg_color);
 	cur_x += 8;
   }
 }
 ...
 void drv_lcd_line(long x_1,long y_1,long x_2,long y_2,char line_type,long color)
 {
   ...
   if(dx>dy)
   {
 	...
 	while(x<=x2)
 	{
 	  ...
 	  {
 	    if(xx1==xx2)
 	    drv_lcd_plot_dot_hor(x+tick,y,color);
 		else
 		drv_lcd_plot_dot_hor(x,y+tick,color);
 	  }
 ...
 void drv_lcd_rectan(long x1,long y1,long x2,long y2,char tick,long color,char fill)
 {
   ...
   if(fill==1)
   {
 	...
 	{
 	  for(i=ymin; i<=ymax; i++)
 	  drv_lcd_plot_dot_hor(xmin,i,color);
 	}
   }
   else
   {
	drv_lcd_line(x1,y1-tk,x2,y1-tk,tick,color);  													//Line Ho.r Top
	drv_lcd_line(x1,y2,x2,y2,tick,color);  															//Line Hor. Botton
	drv_lcd_line(x1,y1,x1,y2,tick,color);  															//Line Ver. Left
	drv_lcd_line(x2-tk,y1,x2-tk,y2,tick,color);  													//Line Ver. Right
   }
 }
 ...
 {
   drv_lcd_text_7x11_hor(ch1,cur_x,cur_y,fg_color,bg_color);
   cur_x += 8;
   drv_lcd_text_7x11_hor(ch2,cur_x,cur_y,fg_color,bg_color);
   cur_x += 8;
   drv_lcd_text_7x11_hor(ch3,cur_x,cur_y,fg_color,bg_color);
   cur_x += 8;
 }
